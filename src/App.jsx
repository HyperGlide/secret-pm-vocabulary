...
import { getAllTerms, onboardingOptions, rooms, scenarios } from "./data";
...
export default function App() {
  ...
  const reviewedKeys = useMemo(
    () => new Set(progress.termsExplored),
    [progress.termsExplored]
  );
  ...
  const totalSteps = allTerms.length + scenarios.length;
  const completionRate = Math.round(
    ((progress.termsExplored.length + progress.scenariosCompleted.length) / totalSteps) * 100
  );
  ...
  const markReviewedFromFlashcards = (roomId, term) => {
    setProgress((prev) => ({
      ...prev,
      termsExplored: [...new Set([...prev.termsExplored, `${roomId}:${term}`])]
    }));
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <a
        href="#main-content"
        className="sr-only left-4 top-4 z-[100] rounded-lg bg-amber-400 px-4 py-2 text-sm font-semibold text-slate-900 focus:fixed focus:block focus:not-sr-only"
      >
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-6 md:gap-8 md:px-8 md:py-8">
        <header className="rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900 to-slate-800 p-6 shadow-2xl">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            <div className="rounded-xl border border-white/10 bg-white/5 p-4">
              <p className="text-sm text-slate-400">Terms in deck</p>
              <p className="text-3xl font-semibold">{allTerms.length}</p>
            </div>
            <div className="rounded-xl border border-white/10 bg-white/5 p-4">
              <p className="text-sm text-slate-400">Terms saved to progress</p>
        ...
        <nav
          className="sticky top-0 z-40 flex flex-wrap gap-2 rounded-xl border border-white/10 bg-slate-950/90 p-2 shadow-lg backdrop-blur-md md:gap-1"
          aria-label="Primary"
        >
          {TABS.map((t) => (
            <button
              ...
              type="button"
              onClick={() => setTab(t.id)}
              className={`rounded-lg px-3 py-2 text-sm font-semibold transition md:px-4 ${
                tab === t.id
                  ? "bg-amber-400/20 text-amber-100 ring-1 ring-amber-400/50"
        ...
        <main id="main-content" className="flex flex-col gap-8 pb-16">
          {tab === "rooms" && (
            <>
              ...
              <section className="grid gap-6 lg:grid-cols-5">
                <div className="lg:col-span-2">
                  ...
                  <p className="mb-3 text-sm text-slate-400">
                    Pick a room to browse weak vs strong phrasing. Hovering a term counts it toward progress.
                  </p>
                  <div className="grid gap-3">
                    {rooms.map((room) => (
                      <button
                        ...
                        className={`rounded-xl border bg-gradient-to-br p-4 text-left transition hover:scale-[1.01] active:scale-[0.99] ${room.color} ${room.border}`}
                      >
                        ...
                        <p className="mt-1 text-sm text-slate-200/90">{room.terms.length} terms</p>
                      </button>
                    ))}
                  </div>
                </div>
          ...
          {tab === "flashcards" && (
            <section
              className="rounded-2xl border border-white/10 bg-slate-900 p-6 md:p-8"
              aria-labelledby="flashcards-heading"
            >
              <h2 id="flashcards-heading" className="text-xl font-semibold">
                Flashcards
              </h2>
              <p className="mt-2 max-w-2xl text-sm text-slate-400">
                Real flip cards for every acronym in the deck. On the back: weak vs strong examples plus
                Wikipedia (article or search) and a web search link for deeper reading.
              </p>
              <div className="mt-8">
                <Flashcards
                  terms={allTerms}
                  onMarkReviewed={markReviewedFromFlashcards}
          ...
          {tab === "translator" && (
            <section className="rounded-2xl border border-white/10 bg-slate-900 p-6" aria-labelledby="translator-heading">
              <h2 id="translator-heading" className="text-xl font-semibold">
                Vocabulary Translator
              </h2>
              <p className="mt-1 text-slate-300">
                Type a casual answer. Compare how each room would phrase the same idea.
              </p>
              <label htmlFor="translator-input" className="mt-4 block text-sm font-medium text-slate-400">
                Your phrasing
              </label>
              <textarea
                id="translator-input"
                className="mt-2 h-28 w-full rounded-xl border border-white/20 bg-slate-800 p-3 text-sm outline-none transition focus:border-amber-400 focus:ring-1 focus:ring-amber-400"
          ...
          {tab === "scenarios" && (
            <section className="rounded-2xl border border-white/10 bg-slate-900 p-6" aria-labelledby="scenarios-heading">
              <h2 id="scenarios-heading" className="text-xl font-semibold">
                Scenario practice
              </h2>
              <p className="mt-2 text-slate-400">
                Answer as you would in an interview, then check which room your language signals.
              </p>
              <p className="mt-4 text-lg text-slate-200">{scenario.prompt}</p>
              <label htmlFor="scenario-input" className="mt-4 block text-sm font-medium text-slate-400">
                Your answer
              </label>
              <textarea
                id="scenario-input"
                ...
                className="mt-2 h-28 w-full rounded-xl border border-white/20 bg-slate-800 p-3 text-sm outline-none transition focus:border-violet-400 focus:ring-1 focus:ring-violet-400"
              ...
              {resultRoom && (
                <div
                  className="mt-4 rounded-xl border border-emerald-300/40 bg-emerald-400/10 p-4 text-sm"
                  role="status"
                >
                  <p>
                  ...
                  <p className="mt-1 text-emerald-100">
                    Suggested upgrade: include {scenario.hints.join(", ")} to signal{" "}
                    {roomBadge(scenario.idealRoom)} language.
                  </p>
                </div>
              )}
            </section>
